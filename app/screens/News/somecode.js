...
export default function SingleChat() {
  ...
  useEffect(() => {
    if (user) {
      //get users message
      const room_name =
        logged_in_user.user > user.more.user
          ? `${logged_in_user.user}and${user.more.user}`
          : `${user.more.user}and${logged_in_user.user}`;

      dispatch(get_old_chats(`?room_name=${room_name}`));

      var ws = new WebSocket(
        `wss://${sitename}/ws/chat/${tenantName}/${room_name}/`
      );
      setWeb_socket(ws);
      ws.onopen = (e) => {
        // connection opened
        ...
        <Grid container>
          ...
          >
            ...
            </Grid>
            {
              ...
              logged_in_user
                ? members
                    .filter((value) => logged_in_user.user != value.user)
                    .map((data) => {
                      return {
                        more: data,
                        message: "",
                    ...
                    .map((e, index) => (
                      ...
                      >
                        <ChatCard
                          bg={
                            e.id == user.id ? "light-green-bg" : "light-grey-bg"
                          }
                          time={e.time}
                          date={e.date}
                          image={e.image}
  ...
  return (
    <Grid item lg={8}>
      ...
      <Grid
        ...
        lg={12}
        sx={{ height: "75vh" }}
        className="rounded-corners light-green-bg"
        style={{ padding: "0px" }}
      >
        ...
        <Grid
          ...
          position="fixed"
          bottom="10px"
          className="rounded-corners light-grey-bg"
        >
